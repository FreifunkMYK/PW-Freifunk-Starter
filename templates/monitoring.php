...
function format_date($zeit) {
    ...
    if ($tag > 0) return "$tag Tag(e)";
    if ($std > 0) return "$std Stunde(n)";
    if ($min > 0) return "$min Minute(n)";
    if ($sek > 0) return "$sek Sekunde(n)";
    if ($sek < 0 or $sek = 0) return "einige Sekunden";
}
